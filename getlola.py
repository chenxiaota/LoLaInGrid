...
def get_cell_lo_la(cityId,dayID):
    ...
    print(sqlGetCellIdLoLa)
    listCellLoLa = Oracle_Query(sqlGetCellIdLoLa)
    return  listCellLoLa

#判断小区的结果并录入中间表
def in_cell_mid_table(cellLoLa,orgIdNamePoint,orgLevel):
    """
    ...
    for cellId, cellLo, cellLa in cellLoLa: #获取基站的经纬度
        for ord_id, org_name, org_point in orgIdNamePoint:   #获取网格的相关内容 (id、name、point list)
            ...
            if judge_result:
                sql_insert_b_cell_mid = "insert into b_cell_mid (org_name,grid_id,org_level,cell_id) " \
                                                "values ('%s','%s','%s','%s')" % (org_name,ord_id,str(orgLevel),cellId)
